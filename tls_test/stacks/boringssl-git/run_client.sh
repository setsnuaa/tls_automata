...
ncat -c "while read server port; do ./build/tool/bssl s_client -min-version $VERSION -max-version $VERSION $CIPHERS -connect \$server:\$port -root-certs /usr/share/ca-certificates/mozilla/Trustis_FPS_Root_CA.crt < /dev/null > /dev/null 2> /dev/null; done" -l -p "$PORT" -k
