...
ncat -c "while read server port; do openssl s_client $VERSION $CIPHERS $CAFILE -connect \$server:\$port < /dev/null > /dev/null 2> /dev/null; done" -l -p "$PORT" -k
