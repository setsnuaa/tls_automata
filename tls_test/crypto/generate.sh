...
openssl req -new \
            -config tls-client.conf \
            -out certs/trusted-client.csr \
            -keyout certs/trusted-client.key \
            -subj "/DC=fr/O=TLS-inferer GASP Inc/CN=Client TLS-inferer GASP"

# Create TLS client certificate
printf "y\ny" | openssl ca -config trusted-root-ca.conf \
                           -in certs/trusted-client.csr \
                           -out certs/trusted-client.crt \
...
openssl req -new \
            ...
            -subj "/DC=fr/O=Untrusted TLS-inferer GASP Inc/CN=Untrusted Client TLS-inferer GASP"
